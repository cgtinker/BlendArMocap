...
class CgtDriver(object):
    ...
    def __init__(self,
                 ...
                 property_value: Optional[float] = -1.,
        ...
        Create a driver using a unique property name as custom data to avoid overwrites.
           ...
           :param property_value: Optional[float]: Value of property (Overwrite prevention as value!:))
    ...
    def set_driver_variables(self):
        ...
        if not cgt_object_prop.set_custom_property(
                obj=self.target_obj, prop_name=self.property_name, value=self.property_value):
